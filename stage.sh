...
S3PATH=$3
STACK_NAME="personalize-connectors"
...
echo "Syncing Glue scripts to staging bucket"
aws s3 sync src/glue s3://${BUCKET}/${S3PATH}glue/

echo "For CloudFormation: https://${BUCKET_DOMAIN}/${BUCKET}/${S3PATH}template.yaml"
echo "Launch CloudFormation stack: https://console.aws.amazon.com/cloudformation/home?region=${BUCKET_LOCATION}#/stacks/create/review?templateURL=https://${BUCKET_DOMAIN}/${BUCKET}/${S3PATH}template.yaml&stackName=${STACK_NAME}"